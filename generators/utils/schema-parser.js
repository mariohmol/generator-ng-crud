...
const serializeFunc = require('./serialize');
            ...
            try {
                if (!fs.lstatSync(pathToSchemaDir).isDirectory())
                    return reject(
                        ...
                        path.resolve(pathToSchemaDir)
                    );
            } catch (e) {
                return reject(
                    'Schema directory not found: ' + path.resolve(pathToSchemaDir)
                );
            }
...
function _loadSchemas(schemaDir, collectionName, baseUri) {
    ...
    var promise = new Promise(function (resolve, reject) {
        ...
        if (schemas != null) return resolve(schemas);
        ...
        glob(schemaDir + '/**/*.json', {})
            .then(function (files) {
                files.forEach(function (pathToFile) {
                    var filePromise = new Promise(function (
                        filePromiseResolve,
                        filePromiseReject
                    ) {
                        /**
                        ...
                        if (
                            path.resolve(path.dirname(pathToFile)) != path.resolve(schemaDir)
                        ) {
                            schemaCollectionName = '';
                            var dirs = path
                                .relative(schemaDir, path.dirname(pathToFile))
                                .split(/\//);
                            schemaCollectionName = camelCase(
                                pluralize.plural(dirs[dirs.length - 1])
                            );
                        }

                        _convertJsonSchemaToModel(
                            ...
                            .catch(function (err) {
                                console.error(
                                    "Error loading schema '" + modelName + "'!\n",
                                    err
                                );
...
function _convertJsonSchemaToModel(
    pathToJsonSchema,
    modelName,
    collectionName,
    baseUri
        ...
        refParser
            ...
            .then(function (dereferencedJsonSchema) {
                ...
                if (process.env.REPLACE_REF) {
                    refSafeSchema = _replaceSchemaRefs(
                        pathToJsonSchema,
                        refSafeSchema,
                        process.env.REPLACE_REF.toLowerCase(),
                        replacedReferences
                    );
                } else {
                    try {
                        ...
                        if (e == 'TypeError: Converting circular structure to JSON') {
                            ...
                            console.log(
                                '@WARNING All $ref values converted to be objects will not be validated.'
                            );
                        } else {
                            console.log(
                                '@WARNING Unexpected error parsing references in ' +
                                pathToJsonSchema
                            );
                        }
                ...
                var mongooseSchema = createMongooseSchema(
                    refs,
                    dereferencedJsonSchemaForMongoose
                );
                ...
                schema.pre('save', function (next) {
                    if (!this.isNew) this._updated = new Date();
...
function _validate(objectToValidate, pathToJsonSchema, baseUri, next) {
    ...
    if (!validation.valid) {
        ...
        validation.errors.forEach(function (error) {
            ...
            if (error.code == 12) errorsToIgnore++;
        });
...
function _replaceSchemaRefs(
    pathToSchema,
    schema,
    replaceWith,
    replacedReferences
) {
    if (!replaceWith) replaceWith = 'object';
    ...
    return traverse(schema).map(function (node) {
        if (node['$ref']) {
            ...
            if (replaceWith == 'objectid') {
                console.log(
                    ' \\_ ' +
                    path.basename(pathToSchema) +
                    ': Reference to ' +
                    node['$ref'] +
                    ' at ' +
                    this.path.join('.') +
                    ' converted to Object ID.'
                );
                this.update({
                    type: 'string',
                    format: 'objectid',
                    ...
                    description:
                        'The ObjectID of an object in the database matching the schema ' +
                        node['$ref']
                });
                ...
                this.update({
                    ...
                    format: 'uri',
                    description:
                        'The URL of a resource matching the schema ' + node['$ref']
                });
            } else {
                console.log(
                    ' \\_ ' +
                ...
                this.update({
                    ...
                    description: 'An object matching the schema ' + node['$ref']
                });
            }
        }
    });
